...
class TankMain(object):
	width  = 600
        path = sys.path[0].split(',')[0]
        path = path.split('tank')[0] +'tankgame/'
	height = 500
	mytank = None
	my_cannon_list = []
	explode_list = []
	...
	def start_game(self):
		...
		for i in range(1,TankMain.tank_num + 1):
		...
		TankMain.walls.append(Wall(screen,250,180,100,20))
		...
		while True:
			...
			for e in TankMain.explode_list:
				e.display()

			if len(TankMain.enemy_list) < TankMain.tank_num:
				TankMain.enemy_list.add(Enemy_Tank(screen))
                                TankMain.terminate_number += 1
	...
	def show_info(self):
		font = pygame.font.SysFont('microsoftyahei', 25)
                text_sf = font.render("kills:%d"%TankMain.terminate_number, True, (255, 0, 0))
		return text_sf
...
class Cannon(BaseItem):
	...
	def __init__(self, screen, tank):
		...
		self.image= pygame.image.load('images/cannonenemy.png')
		self.live = True
		self.rect = self.image.get_rect()
	...
	def hit(self,mytank):
		...
		for hit in hit_list:
			...
			e = Explode(self.screen, hit.rect)
			e.live = True
			TankMain.explode_list.append(e)
...
class Enemy_Cannon(Cannon):
	...
	def hit(self,mytank):
		...
		for hit in hit_list:
			...
			e = Explode(self.screen, hit.rect)
			e.live = True
			TankMain.e_list.append(e)

class Player_Cannon(Cannon):
	def __init__(self,screen,tank):
		Cannon.__init__(self,screen,tank)
		#super(Cannon, self).__init__()
		self.image= pygame.image.load('images/fivestar1.png')
		#self.rect = tank.rect
...
class Explode(BaseItem):
	def __init__(self, screen, rect):
		...
		self.live = False
		self.screen = screen
		self.images = [            pygame.image.load(TankMain.path +'/images/e1.png'),\
					   pygame.image.load(TankMain.path +'/images/e2.png'), \
					   pygame.image.load(TankMain.path +'/images/e3.png'), \
					   pygame.image.load(TankMain.path +'/images/e4.png'), \
					   pygame.image.load(TankMain.path +'/images/e5.png'), \
					   pygame.image.load(TankMain.path +'/images/e6.png'), \
					   pygame.image.load(TankMain.path +'/images/e7.png'),\
					   pygame.image.load(TankMain.path +'/images/e8.png'),\
					   pygame.image.load(TankMain.path +'/images/e9.png')]
		self.image = None
